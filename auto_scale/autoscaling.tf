...
resource "aws_elb" "eng114_david_loadbalancer" {
  ...
  internal           = false
  security_groups    = ["${module.vpc.aws_security_group.eng114_david_sg_app_terra.id}"]
  subnets            = [
    "${module.vpc.aws_subnet.public_subnet.id}",
    "${module.vpc.aws_subnet.public_subnet2.id}"
  ]
...
resource "aws_launch_configuration" "eng114_david_aws_launch" {
  ...
  key_name = var.key_idprivate_subnet
